...
React Flow transport 已进一步单独收口为设计合同：clone 与 Open Canvas 都锁定 `@xyflow/react@12.11.1`，但 union 中的 node/edge `add/remove/replace` 是语义 graph change，不应因为框架 reducer 支持就从 callback 旁路命名 command；edge 在该版本没有非 selection 的 T1 variant。Selection、node position 和 passive measurement 必须先整批分类，再基于 current active-canvas snapshot 提交；mixed invalid batch 不允许先改 selection，render closure 也不能作为 whole-array 写回权威。精确 taxonomy、reducer precedence、history/runtime-field sanitation、`REACT-FLOW-CHANGES-01` 和 `LIBTV-VR-016` 见 [`LIBTV_REACT_FLOW_CHANGE_ROUTING_CONTRACT.md`](research/LIBTV_REACT_FLOW_CHANGE_ROUTING_CONTRACT.md)；runtime 目前仍 generic/partial。

多画布 lifecycle 也不能只看 `canvases[] + activeCanvasId`：当前 graph/viewport/history 已按 canvas 保存，selection 会在切换时清空，Batch 58 的四类 node-bound owner 也会失效；但 `setActiveCanvas` 不拒绝未知 ID，route-local organize/drag/connection/viewport transient 没有 canvas owner，demo canvas 的 responsive preset 会重写存储 viewport，delayed action 仍可能在执行时 late-read active canvas。Open Canvas 的 URL canvasId、summary/document 分层、hydrate 全量替换和 delete-run cleanup 可借方法；其 old-route in-flight save response 没有 expected current canvas guard，又是不能照抄的反例。完整 owner manifest、CRUD/switch plan、`CANVAS-LIFECYCLE-01` 和 `LIBTV-VR-017` 见 [`LIBTV_MULTI_CANVAS_LIFECYCLE_ISOLATION_CONTRACT.md`](research/LIBTV_MULTI_CANVAS_LIFECYCLE_ISOLATION_CONTRACT.md)；runtime 目前 partial，最终删除/fallback/panel/resource 语义仍待 source/product 决定。
...
Selection、focus 和 keyboard context 也必须分权：当前 node selection 已是 active-session IDs，edge selection 却仍留在 stored edge `selected`；`onNodesChange/onSelectionChange/onNodeClick` 三条入口没有共同 primary 规则。Batch 50 已隔离 Director 与普通 page shortcuts，但 Character/History、Shortcuts、Canvas dropdown、node editor 和 local capture Escape 仍依赖分散 listener/focus 规则。Open Canvas 可借 local editor/clipboard guard，selected flags/conflict coupling、weak Escape 和 framework default destructive key 是反例。统一 node/edge/primary selection、focus zone、surface command policy、one-Escape、focus return、`SELECTION-FOCUS-CONTEXT-01` 和 `LIBTV-VR-019` 见 [`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md`](research/LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md)；精确 LibTV modal/shortcut/focus visual 仍需 source evidence。
