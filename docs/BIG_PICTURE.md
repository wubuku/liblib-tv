...
漂移（旧 gate 已被 current manifest 取代），待 replacement 协议处置。
本系列无回归。证据见
[`research/liblib-canvas-batch108-2026-09-05/`](research/liblib-canvas-batch108-2026-09-05/README.md)。
...
命令反馈也不是“需要时弹一条 toast”：VR-018 已把本地命令反馈收口为 catalog + 投影——`libtvCommandFeedback.ts` 持有全部常规命令 surface 的清单与 `projectLibTVCommandFeedback`/`formatLibTVCommandStatus` 稳定投影，所有 status-line surface（Add Node、Share overlay、Agent drawer 状态行、智能剪辑、项目菜单、空画布快速芯片）都带显式 disposition tone（prototype-unavailable → diagnostic，本地边界受理 → positive，导航/无操作 → 静默或 inert），copy 与几何保持源站/原边界（batch 467/477/493/501/504/505）。唯一未决面是 graph connection：validator 已返回 typed rejection reason，但 page `onConnect` 仍静默丢弃，其 source feedback / gesture cleanup / aria / surface projection 因源站劣化（BLOCKED_SOURCE）未决。VideoNode 的 timer chip 由 node-owner 绑定与防交叉清除构造保证（batch 472 审计），Director 保留更完整的 progress/error/retry surface，async run/progress 仍归异步合同岛。Open Canvas 可借的是 transient toast、node status/error、save/conflict banner 和 form field error 的分层；其中文 message 字符串匹配 i18n、无 canvas/operation owner 的 async toast 是反例。Outcome disposition、reason/copy 分离、primary surface、owner lifecycle、`COMMAND-FEEDBACK-01` 和 `LIBTV-VR-018` 见 [`LIBTV_COMMAND_OUTCOME_FEEDBACK_CONTRACT.md`](research/LIBTV_COMMAND_OUTCOME_FEEDBACK_CONTRACT.md)；精确 LibTV toast/invalid style/timeout 仍需 source evidence，普通 LibTV route 不自动复用 FrameOS toast。
