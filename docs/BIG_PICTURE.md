...
Selection、focus 和 keyboard context 也必须分权：Batch 61 已让 node/edge selection 共用 active-session authority，并以 React Flow change routing 作为唯一 framework selection ingress；但 primary identity、plain/additive/marquee precedence、mixed node+edge editor gating、pane/Escape cleanup 和 command snapshot 仍未统一。Batch 50 已隔离 Director 与普通 page shortcuts，但 Character/History、Shortcuts、Canvas dropdown、node editor 和 local capture Escape 仍依赖分散 listener/focus 规则。Open Canvas 可借 local editor/clipboard guard，selected flags/conflict coupling、weak Escape 和 framework default destructive key 是反例。统一 primary selection、focus zone、surface command policy、one-Escape、focus return、`SELECTION-FOCUS-CONTEXT-01` 和 `LIBTV-VR-019` 见 [`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md`](research/LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md)；精确 LibTV modal/shortcut/focus visual 仍需 source evidence。
