...
- `929x874` 下形成与原站同构的素材 → 执行/分镜 → 分组/视频 → 剧本拓扑；
- 整理缩放为 `28%`，关键节点屏幕位置进入 `3px` 自动化容差；
- 确认卡固定左下，约 `168x88`，文案与按钮分两行；
- 未知顶层节点使用稳定三列 fallback，child 保留相对 parent 坐标；
- 还原、保留、保留后 undo/redo 均已验证；
- `929x874`、`1440x900`、`390x844` 三视口无页面横向溢出；
- Batch4-Batch7 与 `npm run check` 全部通过。
