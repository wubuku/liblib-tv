...
| Viewport/coordinate/gesture/placement boundary | `page.tsx` React Flow host/viewport/gesture callbacks + `canvasStore` viewport/add/placement actions + selected overlays | [`LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md`](../LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md`](../LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md)、[`LibTVOverlayPositioning.contract.md`](LibTVOverlayPositioning.contract.md)、Batch 6/7/16/51/60 | `LIBTV-VR-020` planned | `DESIGN_SPEC_COMPLETE` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | V/H/Space cleanup、per-canvas viewport、drag history and overlay formulas are islands；default add still uses browser window center，live/stable phase、host epoch、generation-bound gesture/placement and resize reconcile remain missing。 |
...
| `AssetManagerPanel` | `src/components/AssetManagerPanel.tsx` | [`AssetManagerPanel.spec.md`](AssetManagerPanel.spec.md)、[`LIBTV_UI_OVERLAY_RUNTIME_CATALOG.md`](../LIBTV_UI_OVERLAY_RUNTIME_CATALOG.md)、[`LIBTV_MEDIA_INGRESS_RESOURCE_STATIC_AUDIT_2026-08-27.md`](../LIBTV_MEDIA_INGRESS_RESOURCE_STATIC_AUDIT_2026-08-27.md) | Batch 1/资产抽屉 records；`LIBTV-VR-021` planned | `SPEC_COMPLETE` / `SOURCE_PARITY_PARTIAL` | canvas/assets tab、树、selection 与 drawer->dropdown transition 要作为一个状态机阅读；source Canvas index 与 Personal/Agent reusable Assets 分域，graph node delete 不能推导 asset delete。 |
