...
| 视频/图片节点 | click 替换按钮 (右上角) | 触发 `<input type="file">` 文件选择，选中即以 objectURL 更新节点内容（Batch 203 起图片节点可用，且可撤销）。Batch 236：按钮尺寸随画布缩放（源站 30px × canvas-zoom 实测，图片/视频/音频替换按钮统一） |
...
| 视频节点 (空) 面板 (2026-09-26 源站实测, Batch 239) | 选中空视频 | 下方视频面板 `.bottom-generator.mode-video`：参考 tile + 模式 tabs [全能参考(默认)|首尾帧|视频编辑] + 占位 描述你想要的视频，@引用素材 + Wan 3.0 + 16:9 · 720p · 5s + 积分 300 + ↑ 生成视频 |
...
| 面板 | 出现条件 | 位置 | 内容 |
|---|---|---|---|
| `FrameosNodeFloatingToolbar` | 节点选中 | 节点正上方 57px 处，居中 | 文本：[全屏查看/下载]（Batch 158/171）；空图片：无工具条（Batch 172）；内容图片：**⛶全屏查看/下载/⭐收藏/超清/720全景/打光/改图/裁剪/标注/宫格切分∨** 十项（Batch 220）；**空视频：无工具条（2026-09-26 实测，Batch 238，旧两项为 2026-09-23 旧版采样）**；**内容视频：全屏查看/下载/收藏/剪辑/裁剪/音视频分离/超清/去字幕/片段重拍 九项**（2026-09-25 实测，Batch 225）；内容音频：[下载/收藏]（Batch 222） |
...
| `FrameosGroupToolbar` (Batch 229) | 框选选中 ≥2 节点 | 选中集合包围盒上方 15px，水平居中 | `.group-toolbar` 形态：[成组 | 批量下载]（高 36）；多选时单节点工具条隐藏；点选区包围盒外空白清除。成组为 mock（源站点击效果未采样）；**批量下载=逐个下载选中节点媒体（Batch 234 实现，toast 报数量）** |
...
| `-` / `_` | 缩小 |
| `0` | 适应画布 |
| 方向键 | **无行为**（Batch 247 移除发明行为：源站实测方向键不移动选中节点） |
| `Tab` | **无行为**（Batch 247 移除发明行为：源站实测 Tab 不切换选中） |
