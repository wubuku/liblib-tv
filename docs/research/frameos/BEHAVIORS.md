...
| ToolRail "添加节点" (Batch 167 对齐源站逐字) | click | 弹出菜单：组「添加节点」7 项短标签 文本/图片/视频/音频/3D模型/3D导演台/视频剪辑台（Batch 221 起 3D导演台/视频剪辑台创建真实节点：图标+胶囊按钮卡片；音频仍为 mock 提示不建节点）+ 组「添加资源」上传文件（触发系统文件选择，行为与本地上传一致） |
...
| 音频节点 | 选中 | 下方音频面板：参考 按钮 + 占位 描述音乐 / 配音 / 音效 + BGM S6 模型 + 参数 + 积分 100 + ↑ 生成按钮 (Batch 213；真实生成不在原型范围) |
...
| 面板 | 出现条件 | 位置 | 内容 |
|---|---|---|---|
| `FrameosNodeFloatingToolbar` | 节点选中 | 节点正上方 57px 处，居中 | 文本/视频：恰好两个 icon 按钮 **全屏查看/下载**（2026-09-23 源站实测，Batch 158/171）；有内容图片：**⛶全屏查看/下载/⭐收藏/超清/720全景/打光/改图/裁剪/标注/宫格切分∨** 十项（2026-09-24 源站实测，Batch 220），空图片节点无工具条（Batch 172） |
| `FrameosPromptEditor` | 选中图片/视频等非文本节点 | 节点正下方 12px 处居中，**跟随节点与缩放移动**（Batch 180 恢复跟随定位；此前误停靠在右下角） | 内容区占位“描述你想要的图像，@引用素材”+ 全屏编辑；头部 聚焦/故事版/参考 + 引用芯片 + 删除连线/替换参考；底部 模型/档位/高级设置/积分/生成（详见 Batch 159/160） |
