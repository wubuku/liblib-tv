...
| 节点右键菜单 (2026-09-25 源站实测, Batch 226; 旧版 Batch 170) | 节点右键 | 中间行按内容态：**有内容媒体 → 设置为资产图；空媒体/非媒体 → 重新生成(禁用)**，两侧分隔线。内容图片五行 复制 ⌘C / 复制图片 / 创建副本 ⌘D / 设置为资产图 / 删除 ⌫(红)；空图片 复制图片+重新生成均禁用（Batch 170 旧采样=空图片形态）；非媒体四行 复制/创建副本/重新生成(禁)/删除。禁用行点击无效。图片空白右键菜单 整理/重置 另见 Batch 170 |
...
| `FrameosImageLightbox` (Batch 227) | 内容图片工具条 ⛶全屏查看 | 全屏浮层（近黑背景） | 居中 contain 图片（可缩放）+ 顶部中央 [放大/缩小/重置/下载] + 右上 × 关闭；Esc 关闭（2026-09-25 源站 `.lightbox-chrome` 采样） |
