...
- `LIBTV-VR-020` 的 six-domain spatial model、actual host、live/stable/bootstrap viewport、gesture/placement owner、fixture corpus 和 replacement design 已完成；Batch 63 已关闭 actual-host default add 的 focused runtime slice，但整体保持 `RUNTIME_PARTIAL / SOURCE_PARITY_PARTIAL`；不得用该单一 placement pass、controlled viewport 或 Open Canvas Quick Add 推导普通 LibTV 空间权威已经统一；
