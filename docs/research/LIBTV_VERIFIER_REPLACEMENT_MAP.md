...
| 44 | keyframe time 序列（line 334） | **已修复并转绿（Batch 361）**：验证器迁至当前确定性语义——append 纯拼接 → [0..8]（考古：自 8125872 起纯拼接从未变）；现版预设应用不再记录 application 块/状态条，相关失效断言移除。「append 是否应清除冲突键」维持产品待裁决（batch 352 记录） |
