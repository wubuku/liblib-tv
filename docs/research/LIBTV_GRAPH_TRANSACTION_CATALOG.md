...
| `LIBTV-GI-047` | target canvas viewport is switch restore authority | `REQUIRED_CORRECTNESS` / `FOCUSED_RUNTIME_PASS` / `SOURCE_DECISION_REQUIRED` | Batch 65 A/B stored viewport restore and breakpoint preservation | seed preset cannot silently replace user-owned viewport；source exact responsive policy remains open |
...
| `LIBTV-GI-060` | client/local conversion uses the current actual React Flow host | `REQUIRED_CORRECTNESS` / `FOCUSED_RUNTIME_PARTIAL` | Batch 63 default add and Batch 64 Asset resize use actual host | panel/compact layout cannot shift graph placement or preserved anchor away from visible host intent |
| `LIBTV-GI-061` | accepted viewport/coordinate values are finite and owner-valid | `REQUIRED_CORRECTNESS` / `FOCUSED_RUNTIME_PARTIAL` | Batch 65 viewport callback validates finite `x/y` and zoom `0.1..8` with current canvas owner | invalid/stale value leaves graph/selection/history/viewport unchanged；other spatial writers remain partial |
...
| `LIBTV-GI-063` | bootstrap viewport never overwrites user-owned stable viewport | `REQUIRED_CORRECTNESS` / `FOCUSED_RUNTIME_PASS` | Batch 65 page-session bootstrap/stable owner | breakpoint change preserves stored stable viewport；browser resize anchor remains separate |
...
| `LIBTV-GI-065` | viewport、host frame and temporary gesture state stay outside semantic graph history | `REQUIRED_CORRECTNESS` / `CURRENT_CLONE_FACT` / `FOCUSED_RUNTIME_PARTIAL` | current graph history excludes viewport；Batch 64/65 layout/bootstrap/switch paths verify zero graph history | pan/zoom/resize/cancel adds zero graph snapshots |
...
| `LIBTV-GC-077` actual-host center add | asset panel narrows/shifts React Flow host | add ordinary node or Character image | node center aligns actual host center；one graph history；selection exact | Batch 63 focused runtime pass：desktop/mobile、drawer closed/open、invalid zero mutation；source exact add/auto-pan仍 gated |
...
| `LIBTV-GC-081` responsive bootstrap guard | user has stable viewport | breakpoint/layout changes | preserve declared graph anchor；bootstrap not reapplied | Batch 65 focused pass for stored viewport preservation；browser resize anchor separate |
| `LIBTV-GC-082` viewport switch race | A gesture/animation pending | switch B then A callback | B live/stable unchanged；A result stale | Batch 65 current/old canvas callback focused guard pass；generation/animation race remains partial |
...
| `LIBTV-GC-088` host resize overlay composition | selected image + asset/compact layout change | resize host | one current host/live/node frame；existing LibTV toolbar/panel residuals remain in tolerance | Batch 64 passes Asset host-center anchor and toolbar collision slice；selected-overlay/browser-resize composition remains |
| `LIBTV-GC-089` invalid spatial payload | NaN/Infinity/out-of-range zoom/stale host epoch | viewport/placement callback | stable typed reject/stale；zero graph/selection/history/viewport residue | Batch 65 viewport callback finite/range focused pass；host epoch and other placement writers remain missing |
...
Status is `STATIC_AUDIT_COMPLETE / DESIGN_SPEC_COMPLETE / RUNTIME_PARTIAL / SOURCE_PARITY_PARTIAL`。Batch 63 implements actual-host default add；Batch 64 implements Asset drawer host-center anchor reconciliation；Batch 65 implements responsive bootstrap/stored-viewport ownership and current/old canvas callback guards。Current V/H/Space、per-canvas viewport、drag history、derived/duplicate/organize placement and selected-overlay formulas remain positive islands；full live/stable endpoint、generic generation/host epoch、browser resize anchor、full fixture, Quick Add/drop/pending connection and source mutation remain unimplemented or gated。
