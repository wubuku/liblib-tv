...
Status is `STATIC_AUDIT_COMPLETE / DESIGN_SPEC_COMPLETE / RUNTIME_PARTIAL / SOURCE_PARITY_PARTIAL`。Batch 63 implements actual-host default add；Batch 64 implements Asset drawer host-center anchor reconciliation with current operation/canvas/instance/viewport guards and zero graph history。Current V/H/Space、per-canvas viewport、drag history、derived/duplicate/organize placement and selected-overlay formulas remain positive islands；live/stable、generic host epoch、browser resize/bootstrap、full fixture, Quick Add/drop/pending connection and source mutation remain unimplemented or gated。
