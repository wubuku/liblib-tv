...
- No current clone script covers image preview, annotate, element edit, rotate boundary, layer separation, current toolbar action-set width or active-tool replacement.
...
| Current source toolbar is content-sized `1092.5x49` | No clone script; source matrix only | Missing | Clone is still `900.5px` and seven text actions | Add action IDs/order/width/height assertion |
| Active image tool replaces standard double overlay | Source live/bundle action matrix only | Missing | No clone state exists for replacement | Add one focused state-lifecycle test per low-risk tool |
| Preview is page-level read-only overlay | Source live evidence only | Missing | No clone preview state is asserted | Add open/close and unchanged selection/graph test |
