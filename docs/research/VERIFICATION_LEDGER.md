...
| Viewport / coordinate / gesture / placement | `STATIC_CONTRACT_ONLY` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md`](LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md`](LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md)、Open Canvas `OC-053..060`、Batch 6/7/16/51/60、`LIBTV-VR-020` design | controlled/per-canvas viewport、V/H/Space cleanup、drag one-history、organize/derived/duplicate/overlay islands 已存在；Batch 63 next entry is actual-host default add；live/stable split、generation-bound gesture/placement、resize reconcile 和 focused fixture 尚未实现，exact source add/fit/resize behavior partial |
