...
| `LIBTV-VR-024` Director project/session/command authority | `PROJECT_CODEC_FOCUSED_PASS` + `OWNER_SESSION_FOCUSED_PASS` + `AUTHORED_RUNTIME_FOCUSED_PASS` + `HISTORY_DELETE_RUNTIME_MISSING` | [`LIBTV_DIRECTOR_PROJECT_SESSION_AUTHORITY_CONTRACT.md`](LIBTV_DIRECTOR_PROJECT_SESSION_AUTHORITY_CONTRACT.md)、[`LIBTV_DIRECTOR_COMMAND_HISTORY_DELETE_CONTRACT.md`](LIBTV_DIRECTOR_COMMAND_HISTORY_DELETE_CONTRACT.md)、[`LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md`](LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md)、`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01`、Batch 67-69 | Batch 67 已覆盖 strict codec，Batch 68 已覆盖 owner registry/session，Batch 69 已覆盖 authored/runtime projection stability 和 authoring restore；inactive-owner reconciliation、Director history、reference-aware delete、async owner freshness 和 persistence 仍未实现 |
