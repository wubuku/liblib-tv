...
| Selection / focus / command context | `STATIC_CONTRACT_ONLY` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_STATIC_AUDIT_2026-08-27.md`](LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md`](LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md)、Open Canvas `OC-046..052`、Batch 50、`LIBTV-VR-019` design | node session selection、active image capture guard 和 Director background isolation 已存在；edge/primary authority、foreground policy、one-Escape、focus return 和 focused fixture 尚未实现，exact source modal/shortcut/focus 仍 partial |
