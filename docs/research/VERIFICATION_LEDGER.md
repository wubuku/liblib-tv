...
| Director 当前跨批次集成状态 | `HISTORICAL_RECORDED_PASS` + `CURRENT_RELIABILITY_GATES` + `MANIFEST_RECORDED` | [`LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md`](LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md)、[`storyai-3d-director-desk-2026-08-27/PROGRESS_AUDIT_2026-08-27.md`](storyai-3d-director-desk-2026-08-27/PROGRESS_AUDIT_2026-08-27.md)、Batch 35-50/59/67-70 | 17 个历史脚本仍按成本/副作用/当前价值分级；Batch 59 和 Batch 67-70 形成当前低成本可靠性闸门；不能把历史通过汇总成 source parity，也不能把 current gates 推导成 delete/async/persistence 完成 |
| `LIBTV-VR-024` Director project/session/command authority | `PROJECT_CODEC_FOCUSED_PASS` + `OWNER_SESSION_FOCUSED_PASS` + `AUTHORED_RUNTIME_FOCUSED_PASS` + `HISTORY_FOCUSED_PASS` + `DELETE_ASYNC_PERSISTENCE_RUNTIME_MISSING` | [`LIBTV_DIRECTOR_PROJECT_SESSION_AUTHORITY_CONTRACT.md`](LIBTV_DIRECTOR_PROJECT_SESSION_AUTHORITY_CONTRACT.md)、[`LIBTV_DIRECTOR_COMMAND_HISTORY_DELETE_CONTRACT.md`](LIBTV_DIRECTOR_COMMAND_HISTORY_DELETE_CONTRACT.md)、[`LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md`](LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md)、`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01`、Batch 67-70 | Batch 67 已覆盖 strict codec，Batch 68 已覆盖 owner registry/session，Batch 69 已覆盖 authored/runtime projection stability 和 authoring restore，Batch 70 已覆盖 synchronous project-local command/history/gesture；完整 pointer lifecycle、inactive-owner reconciliation、reference-aware delete、async owner freshness 和 persistence 仍未实现 |
