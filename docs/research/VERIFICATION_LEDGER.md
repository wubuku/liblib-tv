...
| Director 当前跨批次集成状态 | `HISTORICAL_RECORDED_PASS` + `CURRENT_RELIABILITY_GATES` + `MANIFEST_RECORDED` | [`LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md`](LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md)、[`storyai-3d-director-desk-2026-08-27/PROGRESS_AUDIT_2026-08-27.md`](storyai-3d-director-desk-2026-08-27/PROGRESS_AUDIT_2026-08-27.md)、Batch 35-50/59/67-74 | 17 个历史脚本仍按成本/副作用/当前价值分级；Batch 59 和 Batch 67-74 形成当前低成本可靠性闸门；不能把历史通过汇总成 source parity，也不能把 Director current gates 推导成 ordinary canvas async/persistence、remote storage、真实资源或 source-exact persistence |
| `LIBTV-VR-024` Director project/session/command authority | `PROJECT_CODEC_FOCUSED_PASS` + `OWNER_SESSION_FOCUSED_PASS` + `AUTHORED_RUNTIME_FOCUSED_PASS` + `HISTORY_FOCUSED_PASS` + `POINTER_LIFECYCLE_FOCUSED_PASS` + `REFERENCE_DELETE_FOCUSED_PASS` + `ASYNC_AUTHORITY_FOCUSED_PASS` + `PERSISTENCE_FOCUSED_PASS` | [`LIBTV_DIRECTOR_PROJECT_SESSION_AUTHORITY_CONTRACT.md`](LIBTV_DIRECTOR_PROJECT_SESSION_AUTHORITY_CONTRACT.md)、[`LIBTV_DIRECTOR_COMMAND_HISTORY_DELETE_CONTRACT.md`](LIBTV_DIRECTOR_COMMAND_HISTORY_DELETE_CONTRACT.md)、[`LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md`](LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md)、`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01`、Batch 67-74 | Batch 67 已覆盖 strict codec，Batch 68 已覆盖 owner registry/session，Batch 69 已覆盖 authored/runtime projection stability 和 authoring restore，Batch 70 已覆盖 synchronous project-local command/history，Batch 71 已覆盖 focused pointer lifecycle，Batch 72 已覆盖 reference-aware delete 与资源闭包，Batch 73 已覆盖 Director capture/export/phone async owner/result/resource convergence，Batch 74 已覆盖 browser-local versioned persistence、strict reload restore、stale save、坏数据零替换和 storage failure continuity；inactive-owner reconciliation、ordinary canvas async/persistence、copy/paste identity remap、remote storage、真实资源 materialization 和 source parity 仍未实现 |
