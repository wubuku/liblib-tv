...
跨批回归 verifier；这些脚本均使用 pure source corpus 或 fresh
BrowserContext，不写截图。
...
| Director 当前跨批次集成状态 | `HISTORICAL_RECORDED_PASS` + `CURRENT_RELIABILITY_GATES` + `FINAL_REGRESSION_RECORDED_PASS` + `MANIFEST_RECORDED` | [`LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md`](LIBTV_DIRECTOR_CURRENT_VERIFIER_MANIFEST.md)、[`liblib-canvas-batch93-2026-08-29/`](liblib-canvas-batch93-2026-08-29/)、[`storyai-3d-director-desk-2026-08-27/PROGRESS_AUDIT_2026-08-27.md`](storyai-3d-director-desk-2026-08-27/PROGRESS_AUDIT_2026-08-27.md)、Batch 35-50/59/67-93 | 历史脚本仍按成本/副作用/当前价值分级；Batch 93 已在固定 `localhost:4317` 上完成 Director 桌面/移动端、普通画布跨批和 Batch 59/67-92 current gates 串行回归；不能把历史通过汇总成 source parity，也不能把 Director current gates 推导成 ordinary canvas async/persistence、remote storage、复杂真实资源或 source-exact persistence |
