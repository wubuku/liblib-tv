...
| 多画布 lifecycle / owner isolation | `STATIC_CONTRACT_ONLY` / `RUNTIME_PARTIAL` | [`LIBTV_MULTI_CANVAS_LIFECYCLE_ISOLATION_CONTRACT.md`](LIBTV_MULTI_CANVAS_LIFECYCLE_ISOLATION_CONTRACT.md)、Open Canvas list/route/hydrate/delete/save audit、Batch 16/58、`LIBTV-VR-017` design | clone 已有 per-canvas graph/viewport/history、switch selection cleanup 和 node-bound UI reconciliation；invalid target、demo viewport overwrite、page transient/late callback、async/resource owner 隔离及 focused fixture 未实现 |
| Command outcome / feedback ownership | `STATIC_CONTRACT_ONLY` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_COMMAND_OUTCOME_FEEDBACK_CONTRACT.md`](LIBTV_COMMAND_OUTCOME_FEEDBACK_CONTRACT.md)、Open Canvas `OC-040..045`、clone local status/timer/Director static audit、`LIBTV-VR-018` design | connection 已返回 stable reason 但 page reject 静默；其余 feedback 仍是 local string/timer/Director islands，共同 adapter、primary owner、clear/retry/dedupe fixture 与 exact source placement/timeout 尚未实现 |
| Selection / focus / command context | `STATIC_CONTRACT_ONLY` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_STATIC_AUDIT_2026-08-27.md`](LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md`](LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_CONTRACT.md)、Open Canvas `OC-046..052`、Batch 50/61、`LIBTV-VR-019` design | node/edge session selection、active image capture guard 和 Director background isolation 已存在；primary authority、foreground policy、one-Escape、focus return 和 focused `VR-019` fixture 尚未实现，exact source modal/shortcut/focus 仍 partial |
| Viewport / coordinate / gesture / placement | `STATIC_CONTRACT_ONLY` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md`](LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md`](LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md)、Open Canvas `OC-053..060`、Batch 6/7/16/51/60、`LIBTV-VR-020` design | controlled/per-canvas viewport、V/H/Space cleanup、drag one-history、organize/derived/duplicate/overlay islands 已存在；actual-host default add、live/stable split、generation-bound gesture/placement、resize reconcile 和 focused fixture 尚未实现，exact source add/fit/resize behavior partial |
| Media ingress / asset-reference / resource lifecycle | `STATIC_CONTRACT_ONLY` / `RUNTIME_MISSING_OR_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_MEDIA_INGRESS_RESOURCE_STATIC_AUDIT_2026-08-27.md`](LIBTV_MEDIA_INGRESS_RESOURCE_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_MEDIA_INGRESS_RESOURCE_LIFECYCLE_CONTRACT.md`](LIBTV_MEDIA_INGRESS_RESOURCE_LIFECYCLE_CONTRACT.md)、Open Canvas `OC-061..070`、source `LIBTV-SRC-MIR-001..006`、Batch 12/15/17/24/40/46/48、`LIBTV-VR-021` design | Add Resource mocks、Shot object URL preview 和 Director data/blob islands 已存在；common classifier/probe/materializer、cohort transaction、asset/reference/lease/reachability owner 与 focused fixture 未实现，source limits/progress/cancel/placement/register/restore/backend partial |
