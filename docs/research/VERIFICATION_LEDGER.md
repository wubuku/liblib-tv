...
| Selection / focus / command context | `STATIC_AUDIT_ONLY` / `RUNTIME_PARTIAL` / `SOURCE_PARITY_PARTIAL` | [`LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_STATIC_AUDIT_2026-08-27.md`](LIBTV_SELECTION_FOCUS_COMMAND_CONTEXT_STATIC_AUDIT_2026-08-27.md)、Open Canvas `OC-046..052`、Batch 50 Director focused verifier | node session selection、active image capture guard 和 Director background isolation 已存在；edge selection authority、foreground surface suspension、focus return、listener precedence、formal fixture 与 `LIBTV-VR-019` 尚未设计/实现 |
