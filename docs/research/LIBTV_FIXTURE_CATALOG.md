...
| `LIBTV-FIX-DIRECTOR-LOCAL-MODEL-01` | `UI_CONSTRUCTED` / `DIRECT_STORE_DRIVEN` | Batch 48/82 verifier + local input | fresh context + cleared clone-owned storage；合法 OBJ/FBX data URL fixture；显式 reset local model key | Batch 48 descriptor import/persistence/refresh/proxy cleanup；Batch 82 typed resource status、attempt freshness、有限 OBJ/FBX materialization、失败保留 proxy、retry/cancel/release 和 UI feedback |
...
`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` 的当前实现状态以 Batch 67-83
