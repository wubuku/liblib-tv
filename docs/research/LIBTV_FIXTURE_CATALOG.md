...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_RUNTIME_MISSING` | Batch 67 pure `DirectorProjectDocumentV1` corpus + future fresh Director page with deterministic owner/session/generation and resource/graph oracles | implemented valid/invalid/future documents；future A/B source nodes and canvas generations、authored/runtime snapshots、command/history/delete reference graph | strict codec 已覆盖；owner registry、async freshness、Director history/reference-aware delete 仍缺；Batch 59 supplies current browser seed |
...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | clone | Batch 67 V1 snapshot with scene、character/prop/camera、group、four track kinds、edited path、resource/capture descriptor；future A/B owner/session/runtime graph | implemented round-trip、unknown/future/duplicate/dangling/non-finite、zero-partial、runtime-field exclusion/order/isolation；future owner/history/delete/async | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_RUNTIME_MISSING` |
