...
`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` 的当前实现状态以 Batch 67-74
专题台账为准：reference-aware delete/resource closure 已有 focused pass；下方
旧的 fixture 说明只保留历史设计语境，不代表 Batch 72-74 仍未实现删除、
Director async authority 或 browser-local persistence。
...
叙述；当前状态以 Batch 72-74 专题台账为准：Director delete、Director
capture/export/phone async authority 和 browser-local project persistence 已
分别通过 focused runtime，普通画布 async/persistence、inactive-owner
reconciliation、copy/paste、remote storage 和 source parity 仍未实现。
