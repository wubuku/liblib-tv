...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_RUNTIME_FOCUSED_PASS` / `HISTORY_DELETE_RUNTIME_MISSING` | Batch 67 pure `DirectorProjectDocumentV1` corpus + Batch 68/69 fresh Director page with deterministic route/canvas/source owner、session/generation、authored/runtime fingerprints and memory capture sidecar | implemented valid/invalid/future documents、A/B source owners、cross-canvas owners、focus/close/reopen/active-delete/duplicate-reset、seek/playback/path sampling and object/camera/pose authoring cases；future async operation owner and command/history/delete reference graph | strict codec、owner registry/session lifecycle 与 authored/runtime split 已覆盖；async freshness、Director history/reference-aware delete、inactive-owner tombstone reconciliation 和 persistence 仍缺 |
...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | clone | Batch 67 V1 snapshot with scene、character/prop/camera、group、four track kinds、edited path、resource/capture descriptor；Batch 68 A/B route/canvas/source owners、session generations、memory capture sidecar；Batch 69 authored/runtime fingerprints | implemented codec round-trip/reject/isolation plus create/focus/restore/close/reopen/active-delete/duplicate-reset/cross-canvas owner isolation and seek/playback/path/object/camera/pose authored stability；future history/delete/async/inactive-owner reconciliation | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_RUNTIME_FOCUSED_PASS` / `HISTORY_DELETE_RUNTIME_MISSING` |
