...
| `LIBTV-FIX-LOCAL-GRAPH-DELETE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`（batch 517：plain 场景聚焦验收——边闭合/选择失效/零残缺 undo-redo；derived/shot/process 聚合修复与 canvas 场景未建——batch 520 探证实为 BLOCKED_BY_FIXTURE：demo 画布无就绪视频，createFirstFrameReference 静默 no-op，需就绪媒体 fixture 方可驱动派生修复路径） | pure delete corpus + fresh Page + deterministic graph/relation/UI owner state | plain/group/selected-edge/derived/shot/process/canvas/media scenes；ready/reject/unknown policy | structural closure、inverse refs、aggregate repair、UI invalidation、resource diagnostics、one-step undo/redo 和 zero-partial |
