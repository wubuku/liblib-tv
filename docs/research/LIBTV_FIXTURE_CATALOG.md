...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_HISTORY_DELETE_RUNTIME_MISSING` | Batch 67 pure `DirectorProjectDocumentV1` corpus + Batch 68 fresh Director page with deterministic route/canvas/source owner、session/generation and memory capture sidecar | implemented valid/invalid/future documents、A/B source owners、cross-canvas owners、focus/close/reopen/active-delete/duplicate-reset cases；future authored/runtime snapshots、async operation owner and command/history/delete reference graph | strict codec 与 owner registry/session lifecycle 已覆盖；authored/runtime split、async freshness、Director history/reference-aware delete、inactive-owner tombstone reconciliation 和 persistence 仍缺 |
...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | clone | Batch 67 V1 snapshot with scene、character/prop/camera、group、four track kinds、edited path、resource/capture descriptor；Batch 68 A/B route/canvas/source owners、session generations、memory capture sidecar；future authored/runtime graph | implemented codec round-trip/reject/isolation plus create/focus/restore/close/reopen/active-delete/duplicate-reset/cross-canvas owner isolation；future authored stability、history/delete/async/inactive-owner reconciliation | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_HISTORY_DELETE_RUNTIME_MISSING` |
...
3. 保持 `PAR-008` 的 document/snapshot、subgraph copy、node data identity、relation-aware delete 与 graph entrypoint authority fixture/result/verifier contract 为 `DESIGN_SPEC_COMPLETE`；Batch 57 structural connection slice 已有实现和记录，其余 runtime 仍 missing/partial，Reference/domain/source invalid lifecycle 不扩展；
