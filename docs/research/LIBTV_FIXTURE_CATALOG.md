...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_RUNTIME_FOCUSED_PASS` / `HISTORY_FOCUSED_PASS` / `POINTER_LIFECYCLE_FOCUSED_PASS` / `REFERENCE_DELETE_FOCUSED_PASS` / `ASYNC_AUTHORITY_FOCUSED_PASS` / `PERSISTENCE_FOCUSED_PASS` / `CLIPBOARD_REMAP_FOCUSED_PASS` | Batch 67 pure `DirectorProjectDocumentV1` corpus + Batch 68-75 fresh Director page with deterministic route/canvas/source owner、session/generation、authored/runtime fingerprints、project-local history、memory capture sidecar、gesture boundaries、reference closure、async operation/resource ledger、browser-local persistence envelope and project-scoped clipboard packet | implemented valid/invalid/future documents、A/B source owners、cross-canvas owners、focus/close/reopen/active-delete/duplicate-reset、seek/playback/path sampling、object/camera/pose authoring、no-op/rejection outcomes、gesture coalescing、undo/redo、Inspector/R3F/free-draw commit/cancel/pointercancel、object/group/camera/track/path/capture/resource delete closure、capture/export/phone async owner/source/attempt/result convergence、reload restore、stale save、runtime/UI/resource-byte exclusion and storage failure、typed clipboard closure/remap、camera detach/freeze、resource alias、repeated offsets and A-B-A/reload isolation；future inactive-owner reconciliation、whole-project duplicate and ordinary canvas persistence | strict codec、owner registry/session lifecycle、authored/runtime split、synchronous command/history、pointer lifecycle、reference-aware delete、Director async-authority、Director browser-local persistence and same-project clipboard-remap slices 已覆盖；ordinary canvas async/persistence、inactive-owner tombstone reconciliation、whole-project duplicate、remote storage 和 source parity 仍缺 |
...
`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` 的当前实现状态以 Batch 67-75
专题台账为准：reference-aware delete/resource closure、Director async authority、
browser-local persistence 和 same-project clipboard remap 均已有 focused pass；
...
上方历史长句中的 `reference-aware delete、async freshness` 是旧版 fixture
叙述；当前状态以 Batch 72-75 专题台账为准：Director delete、Director
capture/export/phone async authority、browser-local project persistence 和
same-project clipboard remap 已分别通过 focused runtime，普通画布
async/persistence、inactive-owner reconciliation、whole-project duplicate、
remote storage 和 source parity 仍未实现。
