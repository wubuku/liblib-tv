...
| `LIBTV-FIX-LOCAL-GRAPH-DELETE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`（batch 517：plain 场景聚焦验收——边闭合/选择失效/零残缺 undo-redo；derived/shot/process 聚合修复与 canvas 场景未建） | pure delete corpus + fresh Page + deterministic graph/relation/UI owner state | plain/group/selected-edge/derived/shot/process/canvas/media scenes；ready/reject/unknown policy | structural closure、inverse refs、aggregate repair、UI invalidation、resource diagnostics、one-step undo/redo 和 zero-partial |
