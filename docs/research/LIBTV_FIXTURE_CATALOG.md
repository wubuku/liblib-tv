...
| `LIBTV-FIX-LOCAL-VIEWPORT-COORDINATE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `RUNTIME_MISSING` | fresh Page + deterministic A/B canvas generations + measured host epochs + nested/top-level nodes + controlled viewport operation clock | identity/translated/0.28/0.5/1/8 zoom、asset-open/compact host、pan/zoom/drag/resize/switch、default/derived/duplicate/organize placement、selected overlay | finite domain conversion、actual-host center、live/stable/bootstrap ownership、stale/cancel/idempotent gesture、zero-history viewport、exact placement/overlay composition |
