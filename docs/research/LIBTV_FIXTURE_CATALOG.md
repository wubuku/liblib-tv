...
| `LIBTV-FIX-LOCAL-GRAPH-DOCUMENT-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `PURE_CODEC_RECORDED_PASS`（batch 513：`libtvGraphDocument.ts` 严格 V1 读侧 + §9.2 纯 corpus 10/10；Slice B/C/D browser 层未建） | pure V1 payload corpus + fresh Page + `LOCAL-EMPTY-01` | empty/demo/group/nested metadata/invalid version-ID-parent-media cases | document codec、strict load、migration boundary、nested history isolation 和 import zero-partial |
