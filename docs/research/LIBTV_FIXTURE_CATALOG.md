...
| `LIBTV-FIX-LOCAL-DIRECTOR-WHOLE-PROJECT-DUPLICATE-01` | `REQUIRED_DISPOSABLE` | Batch 79 planned fresh BrowserContext + deterministic graph/Director owner set；source canvas with multiple `script-execution` roots、Director objects/groups/cameras/tracks/paths/captures/resources、ordinary result provenance、active session/async/persistence state；target is allocated by the duplicate planner | whole-canvas graph and parent/edge remap、multiple Director owner/project allocation、document two-pass identity/reference rewrite、stable-resource alias、non-portable resource block、capture/result provenance policy、clean target session/history/clipboard、source/target persistence isolation、late source completion fence and storage-failure `SESSION_ONLY` result | Batch 79 only has design；fixture, pure planner, browser verifier、ordinary canvas/Director lifecycle coordinator 和 source parity remain unimplemented/unknown |
