...
| `LIBTV-FIX-LOCAL-VIEWPORT-COORDINATE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL` | Batch 63 fresh Page slice + future deterministic A/B canvas generations、measured host epochs、nested/top-level nodes和 controlled viewport operation clock | implemented identity/translated/invalid + desktop/mobile asset-open default add；future 0.28/0.5/8 zoom、pan/zoom/drag/resize/switch、derived/duplicate/organize、selected overlay | Batch 63 finite host-center conversion、one-history/selection/zero-mutation pass；live/stable/bootstrap、stale/cancel/idempotent gesture、zero-history viewport和 full placement/overlay composition仍缺 |
...
| `LIBTV-FIX-LOCAL-VIEWPORT-COORDINATE-01` | clone | A/B canvas/generation、distinct stable viewports、actual host full/asset-open/compact rect、nested/top-level measured nodes、fake viewport operation IDs | conversion round-trip、host-center add、pan/zoom end/cancel、resize reconcile、switch stale callback、drag/organize/placement/overlay composition | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `RUNTIME_MISSING` |
