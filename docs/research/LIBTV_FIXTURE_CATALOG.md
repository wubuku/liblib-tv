...
| `LIBTV-FIX-LOCAL-VIEWPORT-COORDINATE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL` | Batch 63/64 fresh Page slices + future deterministic A/B canvas generations、measured host epochs、nested/top-level nodes和 controlled viewport operation clock | implemented identity/translated/invalid、desktop/mobile asset-open default add and drawer open/close reconciliation；future 0.28/0.5/8 zoom、pan/zoom/drag/browser resize/switch、derived/duplicate/organize、selected overlay | Batch 63 host-center add and Batch 64 Asset host-resize/current-canvas guard pass；live/stable/bootstrap、generic host epoch、cancel/idempotent gesture and full placement/overlay composition仍缺 |
...
| `LIBTV-FIX-LOCAL-VIEWPORT-COORDINATE-01` | clone | Batch 63/64 fresh Page slices + future A/B canvas/generation、distinct stable viewports、actual host full/asset-open/compact rect、nested/top-level measured nodes、fake viewport operation IDs | conversion round-trip、host-center add、Asset resize reconcile/current-canvas guard、future pan/zoom end/cancel、browser resize、switch stale callback、drag/organize/placement/overlay composition | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL` |
