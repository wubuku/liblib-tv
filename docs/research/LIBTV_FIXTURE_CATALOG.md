...
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_RUNTIME_FOCUSED_PASS` / `HISTORY_FOCUSED_PASS` / `POINTER_LIFECYCLE_FOCUSED_PASS` / `REFERENCE_DELETE_FOCUSED_PASS` / `ASYNC_AUTHORITY_FOCUSED_PASS` / `PERSISTENCE_RUNTIME_MISSING` | Batch 67 pure `DirectorProjectDocumentV1` corpus + Batch 68-73 fresh Director page with deterministic route/canvas/source owner、session/generation、authored/runtime fingerprints、project-local history、memory capture sidecar、gesture boundaries、reference closure and async operation/resource ledger | implemented valid/invalid/future documents、A/B source owners、cross-canvas owners、focus/close/reopen/active-delete/duplicate-reset、seek/playback/path sampling、object/camera/pose authoring、no-op/rejection outcomes、gesture coalescing、undo/redo、Inspector/R3F/free-draw commit/cancel/pointercancel、object/group/camera/track/path/capture/resource delete closure、capture/export/phone async owner/source/attempt/result convergence；future inactive-owner reconciliation、copy/paste remap and durable persistence | strict codec、owner registry/session lifecycle、authored/runtime split、synchronous command/history、pointer lifecycle、reference-aware delete and Director async-authority slices 已覆盖；ordinary canvas async、inactive-owner tombstone reconciliation、copy/paste identity remap 和 persistence 仍缺 |
...
`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` 的当前实现状态以 Batch 67-73
专题台账为准：reference-aware delete/resource closure 已有 focused pass；下方
旧的 fixture 说明只保留历史设计语境，不代表 Batch 72/73 仍未实现删除或
Director async authority。
...
叙述；当前状态以 Batch 72/73 专题台账为准：Director delete 和 Director
capture/export/phone async authority 已分别通过 focused runtime，普通画布
async、inactive-owner reconciliation、copy/paste 和 persistence 仍未实现。
