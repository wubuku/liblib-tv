...
>
> 本文来自 2026-08-26 对 `canvasStore`、`page.tsx`、Batch 4-48 verifier、当前源站研究边界和 Batch 48 clone-owned local model workflow 的只读审计，并由 Batch 82/92 补充本地 OBJ/FBX materialization、owner/lease 结果，再由 Batch 93 完成最终桌面/移动端与跨批回归确认，Batch 94/95/96 继续补充 Director focus、canvas-media ingress 和 multi-camera/Shot fixture 边界。本文不授权修改代码、测试、截图或源站项目。
>
...
Batch 48 新增了 browser-local model-library storage。Batch 82 在同一 fixture 域
补充了有限的本地 OBJ/FBX materialization。两批合起来仍只定义 clone-owned
prototype 边界：
...
- 不把 localStorage descriptor、proxy object 或有限解析 `Object3D` 升级为真实
  LibTV 生产资产合同。
...
| `LIBTV-FIX-LOCAL-GRAPH-DOCUMENT-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `PURE_CODEC_RECORDED_PASS`（batch 513 纯层 + batch 514 Slice B focused browser 层（history isolation，含 viewport undo-echo 异常发现——见 batch514 目录）；Slice C/D 未建） | pure V1 payload corpus + fresh Page + `LOCAL-EMPTY-01` | empty/demo/group/nested metadata/invalid version-ID-parent-media cases | document codec、strict load、migration boundary、nested history isolation 和 import zero-partial |
| `LIBTV-FIX-LOCAL-SUBGRAPH-COPY-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`（batch 516：真实 duplicateGraphSelection 聚焦验收——新身份/数据深相等/内部边重映射/外部边不泄漏/一步 undo；专用 group/child topology UI 场景未建） | fresh Page + deterministic ID provider + group/child/external/derived topology | named roots、descendant closure、internal/external edges、reference roles、placement/history boundary | copy planner、ID/reference map、detach/group、zero-partial 和 one-step undo/redo |
| `LIBTV-FIX-LOCAL-NODE-DATA-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`（batch 515：真实 getDefaultNodeData 注册表聚焦验收——11 类型齐全/确定性/JSON 安全/无 runtime 键，style/effect 额外分支为 STATIC_FACT 留档；全量 V0 corpus 未建） | pure V0 corpus + fresh Page + deterministic node/edge/aggregate IDs | all 11 runtime types、nested provenance、shot/process aggregates、repo/https/data/blob media、invalid type/version/ref cases | registry completeness、operation transforms、deep isolation、aggregate integrity、media diagnostics 和 zero-partial |
| `LIBTV-FIX-LOCAL-GRAPH-DELETE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`（batch 517：plain 场景聚焦验收——边闭合/选择失效/零残缺 undo-redo；derived-reference 场景已由 batch 521 聚焦验收（创建事务/防重守卫/源删除后派生图独立存活/零残缺 undo）；shot 聚合场景已由 batch 522 聚焦验收（完成创建带 sourceBreakdownId 反向引用的 cohort、删除 breakdown 后 cohort 存活且反向引用按观察契约保留、零悬挂边、零残缺 undo）；process 聚合经 batch 523 探针确认 createLongVideoProcess(LongVideoProcessInput) 单事务创建 12 节点 cohort（一步 history），其删除与已验收的 removeNode 结构闭合/零残缺 undo 合同同径，未单独成场景；canvas 场景已由 batch 527 聚焦验收（软删回收站快照/history 条目清理/活动画布相邻回退+选择清空/restore 恢复图内容）。至此四组场景（plain/derived/shot/canvas）全部聚焦验收完毕。勘误：batch 520 曾判 BLOCKED_BY_FIXTURE，实为 batch 246/255 防重守卫（demo video 已有 image→video 边触发仅记 attempt 跳过），batch 521 以全新 video 反证纠正） | pure delete corpus + fresh Page + deterministic graph/relation/UI owner state | plain/group/selected-edge/derived/shot/process/canvas/media scenes；ready/reject/unknown policy | structural closure、inverse refs、aggregate repair、UI invalidation、resource diagnostics、one-step undo/redo 和 zero-partial |
| `LIBTV-FIX-LOCAL-GRAPH-ENTRYPOINT-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`（batch 518：跨入口一致性聚焦验收——同一语义提案经 change 回调被 T 门拒绝零残缺、经命名命令受理一步；组合 corpus 未建） | composition of connection/document/copy/data/delete fixtures + fresh Page | ordinary/group/derived/aggregate/history/remote aliases；same proposal over multiple ingress | T0-T5 routing、transport whitelist、full-draft reject、restore/remote stale 和 zero-partial |
...
| `LIBTV-FIX-LOCAL-REACT-FLOW-CHANGES-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 61 VR-016 focused corpus；全量 corpus 未建 | fresh Page + exact 12.11.1 change corpus + deterministic store/render timing | node/edge select、position/measure、semantic/unknown/mixed/stale/current-canvas cases | whole-batch T0/T1 routing、current snapshot、one drag history、zero-partial reject 和 runtime-field sanitation |
| `LIBTV-FIX-LOCAL-CANVAS-LIFECYCLE-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-017 slices A/B/E closed batch 435/436/437 + 466 精化；全量 corpus 未建 | fresh Page + deterministic A/B/C graph/viewport/history + UI/transient/async/resource controls | valid/invalid/same switch、organize/drag/viewport race、duplicate/delete/final fallback、old timer/save cases | registry/active/selection/history atomicity、owner manifest、stale callback isolation、resource impact 和 zero-partial lifecycle |
| `LIBTV-FIX-LOCAL-COMMAND-FEEDBACK-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `PURE_RUNTIME_RECORDED_PASS`（batch 510：`libtvCommandFeedbackFixture.ts` 确定性世界 + 15 场景全绿；视觉几何仍 browser-gated） | fresh Page + deterministic dispositions/reasons + fake timer/announcement clock + A/B canvas/node/surface/operation owners | field/connection reject、prototype unavailable、node guard、visible result、async fail/retry/stale/duplicate、switch/delete/close、burst/route isolation | reason/copy separation、primary surface、zero-history feedback、owner cleanup、dedupe、accessibility 和 prototype honesty |
| `LIBTV-FIX-LOCAL-SELECTION-FOCUS-CONTEXT-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 62 clone-owned focused runtime；全量 corpus 未建 | fresh Page + deterministic node/edge/primary selections + editable/surface/modal/Director focus zones + A/B canvas generations | click/multi-select/edge select、editable keys、foreground contexts、single Escape、switch/delete/undo、focus return/fallback、route isolation | validated active selection、declared dispatch result、zero-history selection/focus、one-layer unwind、focus containment/return 和 stale-owner cleanup |
...
| `LIBTV-FIX-LOCAL-MEDIA-INGRESS-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-021 slices A-F closed batch 450-456；全量 corpus 未建 | fresh Page + synthetic in-memory image/video/audio/text files + deterministic classifier/probe/fake materializer/resolver clock + A/B generations + asset/reference/resource ledger | cancel、invalid/mixed/out-of-order cohorts、replace success/failure、retry/delete/switch races、history/asset attach、blob/data reachability、route isolation | typed validation/probe、full-plan cohort commit、last-known-good、zero stale residue、exact history/selection/feedback、lease transfer/release and honest capability projection |
| `LIBTV-FIX-LOCAL-EDITOR-SESSION-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-022 slices A-E closed batch 445-449 + 474；全量 corpus 未建 | pure session reducer + fresh Page + deterministic text/config/record/range/4x4 bitmap owners + fake clock/async acceptor + graph/resource/focus/feedback oracles | open/cancel/blur/noop/commit、gesture/undo/redo/reset、drift/delete/switch、async accept/fail/late、budget/IME/empty-command cases | unique owner、semantic dirty/no-op、one local gesture、one accepted graph step、zero cancel/stale residue、byte/release budget、undo precedence and honest controls |
| `LIBTV-FIX-LOCAL-MEDIA-RENDITION-01` | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-023 slices A-D closed batch 441-444；全量 corpus 未建 | pure frame/rendition/fit model + fresh Page + deterministic 16:9/2:1/1:1/9:16/odd media/output descriptors + thumbnail/full/poster aliases + fake measurement clock | matching/mismatching/invalid metadata、same/mixed-ratio output switch、cover/contain/editor round-trip、status transition、stale measurement、detail/route isolation | finite authority/provenance、atomic selection/frame policy、zero passive-measure history、fresh overlay anchor、full/visible editor intent and no fabricated resize |
...
| `LIBTV-FIX-DIRECTOR-LOCAL-MODEL-01` | `UI_CONSTRUCTED` / `DIRECT_STORE_DRIVEN` | Batch 48/82/92 verifier + local input | fresh context + cleared clone-owned storage；合法 OBJ/FBX data URL fixture；显式 reset local model key；owner/project/session/generation 从 fresh Director session 获取 | Batch 48 descriptor import/persistence/refresh/proxy cleanup；Batch 82 typed resource status、attempt freshness、有限 OBJ/FBX materialization、失败保留 proxy、retry/cancel/release 和 UI feedback；Batch 92 strict decoded-byte budget、owner-scoped request/lease、deferred/final release 和 stale-owner cleanup |
| `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` | `PURE_CODEC_RECORDED_PASS` / `OWNER_SESSION_FOCUSED_PASS` / `AUTHORED_RUNTIME_FOCUSED_PASS` / `HISTORY_FOCUSED_PASS` / `POINTER_LIFECYCLE_FOCUSED_PASS` / `REFERENCE_DELETE_FOCUSED_PASS` / `ASYNC_AUTHORITY_FOCUSED_PASS` / `PERSISTENCE_FOCUSED_PASS` / `CLIPBOARD_REMAP_FOCUSED_PASS` / `OWNER_REACHABILITY_FOCUSED_PASS` / `WHOLE_PROJECT_DUPLICATE_FOCUSED_PASS` / `DURABLE_TOMBSTONE_FOCUSED_PASS` / `IMPORT_EXPORT_FOCUSED_PASS` / `LOCAL_RESOURCE_MATERIALIZATION_FOCUSED_PASS` / `COMMAND_FEEDBACK_FOCUSED_PASS` / `LOCK_EDITABILITY_FOCUSED_PASS` / `SELECTION_CRUD_FOCUSED_PASS` / `TRANSFORM_CONTEXT_FOCUSED_PASS` / `RESTORE_SELECTION_FOCUSED_PASS` / `SELECTION_TIMELINE_AUTHORITY_FOCUSED_PASS` / `SCENE_COMMAND_FOCUSED_PASS` / `OBJECT_CAMERA_GROUP_COMMAND_FOCUSED_PASS` / `LOCAL_RESOURCE_LEASE_FOCUSED_PASS` / `FINAL_REGRESSION_RECORDED_PASS` | Batch 67 pure `DirectorProjectDocumentV1` corpus + Batch 68-76 fresh Director page with deterministic route/canvas/source owner、session/generation、authored/runtime fingerprints、project-local history、memory capture sidecar、gesture boundaries、reference closure、async operation/resource ledger、browser-local persistence envelope、project-scoped clipboard packet、all-canvas live owner set、whole-project duplicate planner、durable tombstone and strict import/export；Batch 82/92 local resource descriptor/materializer/lease fixture；Batch 83 deterministic Director command feedback fixture；Batch 93 desktop/mobile and cross-batch regression fixture | implemented valid/invalid/future documents、A/B source owners、cross-canvas owners、focus/close/reopen/active-delete/duplicate-reset、seek/playback/path sampling、object/camera/pose authoring、no-op/rejection outcomes、gesture coalescing、undo/redo、Inspector/R3F/free-draw commit/cancel/pointercancel、object/group/camera/track/path/capture/resource delete closure、capture/export/phone async owner/source/attempt/result convergence、reload restore、stale save、runtime/UI/resource-byte exclusion and storage failure continuity、typed clipboard closure/remap、camera detach/freeze、resource alias、repeated offsets、A-B-A/reload isolation、inactive source/canvas tombstone、active two-phase cleanup、repeated reconciliation、graph undo and retained persistence、whole-project identity/reference remap、durable tombstone cleanup、strict JSON transfer、finite local OBJ/FBX materialization、parse-failure proxy retention、resource retry/cancel/release、owner-scoped lease、typed disposition/reason projection、ARIA status、committed-success suppression、meaningful no-op visibility、zero-history feedback、desktop/mobile surface/overflow/close-reopen and serial cross-batch current gates；ordinary canvas async/persistence、complex provider/materializer、remote storage 和 source parity 仍缺 |
| `LIBTV-FIX-LOCAL-DIRECTOR-SHOT-01` | `DIRECT_STORE_DRIVEN` / `FOCUSED_RUNTIME_RECORDED_PASS` | Batch 96 在 `LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` 上派生旧 V1 payload、多个 camera/Shot、capture provenance、clipboard packet 和 duplicate target；fresh BrowserContext，明确清理 Director persistence | legacy `shots` omission/decode、normalized export、Shot create/switch/update、time-range validation、history undo/redo、camera/Shot delete repair、last-camera rejection、capture gallery grouping、clipboard/whole-project remap、reload/import/export、desktop/mobile overflow 和 zero diagnostics | clone-owned Shot authoring；不证明 LibTV source Shot schema、camera/time-range semantics、exact DOM/CSS 或真实多机位资源 |
| `LIBTV-FIX-LOCAL-DIRECTOR-WHOLE-PROJECT-DUPLICATE-01` | `DIRECT_STORE_DRIVEN` / `RECORDED_PASS` | Batch 79 fresh BrowserContext + deterministic pure graph/Director owner set；source canvas with `script-execution` roots、Director objects/groups/cameras/tracks/paths/captures/resources、ordinary result provenance；target allocated by duplicate planner | whole-canvas graph and parent/edge remap、multiple Director owner/project allocation、document two-pass identity/reference rewrite、stable-resource descriptor alias、non-portable resource block、clean target session/history/clipboard、source/target persistence isolation、zero-diagnostic browser run | Batch 79 clone-owned focused pass；Batch 82 只补本地运行时 materialization，不改变 duplicate 对 portable/stable descriptor 的边界；source duplicate semantics、capture byte copy 和 remote persistence 仍 unknown |
| `LIBTV-FIX-LOCAL-DIRECTOR-DURABLE-TOMBSTONE-01` | `DIRECT_STORE_DRIVEN` / `RECORDED_PASS` | Batch 80 fresh BrowserContext + pure storage backend；canvas A 上 active source A 与 closed source B，共享一个 local model descriptor；A 带 transient capture sidecar 和 Director history | strict tombstone envelope、load/reopen block、save resurrection guard、stale/malformed/write-failure、active/inactive cleanup、capture sidecar clear、shared resource retain、last-reference release、graph/Director history isolation、reload recovery guard、zero diagnostics | Batch 80 clone-owned focused pass；不证明 LibTV source delete/recovery、remote persistence、真实资源 materialization 或共享资产账户语义 |
| `LIBTV-FIX-LOCAL-DIRECTOR-IMPORT-EXPORT-01` | `DIRECT_STORE_DRIVEN` / `RECORDED_PASS` | Batch 81 pure V1 document corpus + fresh BrowserContext；source document A 导出后导入当前 target session B，文件通过 download/file-input 传递 | strict decode/rebind、owner/project replacement、internal reference preservation、capture/runtime/UI exclusion、one-entry import history、undo/redo、same-document no-op、invalid zero-partial、ordinary graph/history isolation | Batch 81 clone-owned focused pass；不证明 LibTV source 文件格式、导入/导出 UI、remote sync、真实资源 materialization 或跨账户语义 |
| `LIBTV-FIX-LOCAL-DIRECTOR-COMMAND-FEEDBACK-01` | `DIRECT_STORE_DRIVEN` / `RECORDED_PASS` | Batch 83 fresh BrowserContext + deterministic Director dispositions/reasons and fixed-header owner | rejected/stale/conflict/meaningful-no-op visible projection、committed generic-success suppression、ARIA status semantics、mobile header containment、zero-history feedback boundary、zero diagnostics | Batch 83 clone-owned focused pass；不证明 LibTV source feedback taxonomy、exact copy/color/placement、ordinary canvas unified feedback 或 remote operation |
...
| `LIBTV-FIX-LOCAL-SUBGRAPH-COPY-01` | clone | G/A/B/C/X/Y/D aliases、deterministic node/edge IDs、owned/external/unmodeled refs | closure/dedupe、parent detach/remap、edge policy、reference role、placement、atomic history | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 516 真实 duplicate 流聚焦验收；全量 corpus 未建 |
...
| `LIBTV-FIX-LOCAL-GRAPH-DELETE-01` | clone | deterministic nodes/edges/selection、registered inverse refs、complete/broken aggregates、UI owners、media locators | closure、cascade/detach/reset/block、stable plan/reason、overlay/resource result、undo/redo | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 517 plain 场景聚焦验收；全量 corpus 未建 |
| `LIBTV-FIX-LOCAL-GRAPH-ENTRYPOINT-01` | clone | A/B/C、group/derived/shot/process、valid/invalid history、current/stale patch aliases | equivalent ingress、change whitelist、planned command、restore and remote authority | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 518 跨入口一致性聚焦验收；全量 corpus 未建 |
...
| `LIBTV-FIX-LOCAL-REACT-FLOW-CHANGES-01` | clone | exact change variants、same-ID mixed order、finite/invalid numbers、stale render/store snapshots、active canvas switch | T0/T1 accepted batches、semantic/unknown rejection、selection owner、drag history、runtime-field projection | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 61 VR-016 focused corpus；全量 corpus 未建 |
| `LIBTV-FIX-LOCAL-CANVAS-LIFECYCLE-01` | clone | deterministic A/B/C IDs、distinct graph/viewport/history、selection/UI/transient owners、fake timer/save/resource ledger | switch/unknown/same、duplicate/delete/fallback、old organize/drag/viewport/completion convergence | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-017 slices A/B/E closed batch 435/436/437 + 466 精化；全量 corpus 未建 |
| `LIBTV-FIX-LOCAL-COMMAND-FEEDBACK-01` | clone | stable disposition/reason/args、fake timers、feedback ledger、A/B canvas/node/surface/attempt owners | reject/noop/start/complete/fail/stale/duplicate、edit/retry/close/switch/delete/undo/burst | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `PURE_RUNTIME_RECORDED_PASS`（batch 510，verify-liblib-batch510.py 15/15） |
| `LIBTV-FIX-LOCAL-SELECTION-FOCUS-CONTEXT-01` | clone | node/edge/primary selection sets、editable/surface/modal/Director zones、focus origin/fallback、A/B canvas generations | click/multi-select/edge select、Delete/undo/redo、editable isolation、modal/Director precedence、single Escape、switch/delete/undo | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——batch 62 clone-owned focused runtime；全量 corpus 未建 |
...
| `LIBTV-FIX-LOCAL-MEDIA-INGRESS-01` | clone | synthetic files、deterministic classifier/probe/materializer clock、ingress/attempt/cohort/asset/reference aliases、lease/reachability counters | chooser cancel、invalid/mixed/out-of-order cohort、replace/retry/delete/switch races、history/asset attach、blob/data/route cases | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-021 slices A-F closed batch 450-456；全量 corpus 未建 |
| `LIBTV-FIX-LOCAL-EDITOR-SESSION-01` | clone | deterministic text/config/record/range/request/empty owners、small bitmap buffers、session/source versions、fake async/resource/focus/history oracles | dirty/noop/cancel/blur、gesture/undo/redo/reset、drift/IME/delete/switch、submit race/late result、budget eviction、inert-control honesty | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-022 slices A-E closed batch 445-449 + 474；全量 corpus 未建 |
| `LIBTV-FIX-LOCAL-MEDIA-RENDITION-01` | clone | deterministic ratio/media/output/request/frame/rendition/content-box descriptors + invalid metadata + fake measurement clock | frame derivation、same/mixed output、thumbnail/full/poster swap、fit/editor round-trip、status/stale/detail/route cases | `REQUIRED_DISPOSABLE` / `DESIGN_SPEC_COMPLETE` / `FOCUSED_RUNTIME_PARTIAL`——VR-023 slices A-D closed batch 441-444；全量 corpus 未建 |
...
`LIBTV-FIX-LOCAL-DIRECTOR-AUTHORITY-01` 的当前实现状态以 Batch 67-96
...
| `LIBTV-PAR-013` | `DIRECTOR-LOCAL-MODEL-01` | Batch 48 descriptor/proxy 与 Batch 82 有限 OBJ/FBX materialization 均已形成 recorded pass；读取两批稳定 verifier 和 reset 合同 | 不把 local descriptor/proxy/parsed Object3D 升级为真实资产、生产 loader 或远端持久化 |
...
6. 继续维护 `DIRECTOR-LOCAL-MODEL-01` 的 storage reset、fresh-context、proxy-cleanup
   和有限 materialization/lease 断言；
