...
| `LIBTV-PAR-008` | `LOCAL-GRAPH-CONNECTION-01` + `LOCAL-GRAPH-DOCUMENT-01` + `LOCAL-SUBGRAPH-COPY-01` + `LOCAL-NODE-DATA-01` + `LOCAL-GRAPH-DELETE-01` + `LOCAL-DEMO/EMPTY/GROUP/DERIVED` | connection structural slice 已由 Batch 57 通过；document/snapshot、subgraph copy、node data identity、relation-aware delete 合同/fixture/`VR-010..013` 仍是设计完成 | source invalid lifecycle/Reference/domain/Option-drag/cascade/detach 需要 `SOURCE-GRAPH-DELETE-01` 等决定；persistence deferred |
...
3. 保持 `PAR-008` 的 document/snapshot、subgraph copy、node data identity 与 relation-aware delete fixture/result/verifier contract 为 `DESIGN_SPEC_COMPLETE / RUNTIME_MISSING`；Batch 57 structural connection slice 已有实现和记录，Reference/domain/source invalid lifecycle 仍不扩展；
