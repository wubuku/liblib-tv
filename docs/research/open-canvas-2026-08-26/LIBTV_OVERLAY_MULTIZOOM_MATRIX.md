...
当前源站在 28% 和 50% 两个直接可见 zoom 档都满足：
...
同时发现一个不能被旧 clone 合同掩盖的差异：顶部工具条到节点顶部的实测 gap 并非固定 `16px`。当前快照在约 28% 为 `16.794px`，在 50% 为 `22px`。这说明顶部 NodeToolbar 的 source positioning 还需要继续从 DOM/style 或更多 zoom 档解释，不能直接把 clone 的 `offset=16` 当作完整 source contract。
...
- viewport：`929x874`；
- 观察日期：2026-08-26；
- 动作：缩放菜单切换 `50%`、`100%`、`适合屏幕`，切换图片节点，读取 DOM rect；
...
| 切换 50% | 屏幕尺寸不变，node 世界尺寸随 zoom 增长 | 反缩放/非缩放层必须各自正确 |
...
| toolbar vertical gap | 28% `16.794`、50% `22` | 验证脚本按 `16` 断言 | 多 zoom 合同不完整 |
...
- 顶部 gap 在 28% 到 50% 间的精确函数尚未建立；当前只有两个直接 live 档，不能做线性外推。
