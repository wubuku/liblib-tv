...
| `OC-BP-004` graph transaction hardening | `OC-ADOPT-004..006` | `LIBTV-PAR-008` | connection + document/snapshot 子切片 `DESIGN_SPEC_COMPLETE / RUNTIME_MISSING`；subgraph copy 仍 `DESIGN_FIRST` | `LIBTV-FIX-LOCAL-GRAPH-CONNECTION-01` + `GRAPH-DOCUMENT-01` 设计完成；运行 fixture/source fixture 待定 | `LIBTV-VR-009..010` 设计完成；Reference/entry-point/invalid lifecycle 仍 source-blocked |
...
可以借：versioned schema、validation pipeline、type compatibility、cycle/self/dangling/duplicate guard、subgraph ID map 和内部边闭包。
...
基础 fixture 使用 `LIBTV-FIX-LOCAL-GRAPH-CONNECTION-01`、`LIBTV-FIX-LOCAL-GRAPH-DOCUMENT-01`、`LIBTV-FIX-LOCAL-EMPTY-01`、`LIBTV-FIX-LOCAL-DEMO-01`、`LIBTV-FIX-LOCAL-GROUP-01` 和 `LIBTV-FIX-LOCAL-DERIVED-01`。前两者的 topology/corpus、scenario reset 和 `LIBTV-VR-009..010` split 已设计完成，但运行 fixture 和 verifier 尚未实现。不能用共享源站验证 destructive graph guard。
...
2. `OC-BP-004` 的 connection 与 document/snapshot schema/result/fixture/`LIBTV-VR-009..010` 设计已完成；下一步保持 runtime 未实现，继续补 Reference/导入/批量/同步 source 决定与 invalid lifecycle，subgraph copy 另走独立设计；
