...
| `OC-ADOPT-006` | connection type compatibility、方向归一化和 DAG guard | `ADOPT_METHOD` | 为 duplicate/dangling/self-loop/cycle guard 提供纯逻辑参考 | `OC-003`；`LIBTV-PAR-008`；`LIBTV-FIX-LOCAL-EMPTY-01` | LibTV 自身的 Handle、edge direction、group/derived 语义先落档；不改变已确认的 edge flow 视觉 |
