...
3. `OC-ADOPT-004..006/016/017/019`：使用已经完成的 graph invariant/case 表、四份 component contracts、delete matrix、[`LIBTV_GRAPH_MUTATION_ENTRYPOINT_TRUST_MATRIX.md`](../LIBTV_GRAPH_MUTATION_ENTRYPOINT_TRUST_MATRIX.md)、[`LIBTV_REACT_FLOW_CHANGE_ROUTING_CONTRACT.md`](../LIBTV_REACT_FLOW_CHANGE_ROUTING_CONTRACT.md) 和两份 dated static audit；按 verifier 分开维护 runtime maturity，保持 persistence deferred，继续取得 Reference、导入/批量/同步、invalid lifecycle、Option-drag、resize/reconnect 和 cascade/detach source/product 决定；
