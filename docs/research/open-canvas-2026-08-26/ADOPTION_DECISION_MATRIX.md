...
| `OC-ADOPT-025` | foreground editor session + local history + graph commit handoff，以及 full-bitmap entry budget、close-first async save、weak freshness/result handling 反例 | `ADAPT_TO_LIBTV` | 为 LibTV 十类普通编辑器定义 session/baseline/draft、native/local/graph undo routing、commit/cancel、async/resource handoff 和 honest disabled control | `OC-071..080`、`OC-TR-018`；`DEC-038`；`LIBTV-PAR-004/008/009/011/015`；`LIBTV-FIX-LOCAL-EDITOR-SESSION-01`；`LIBTV-VR-022` | static/design complete；runtime fragmented/source parity partial；不移植 40-entry full bitmap、JPEG/0.92、close-first、HTML/editor schema 或 node-ID-only patch；任何 runtime slice 需单独授权 |
| `OC-ADOPT-026` | selected output normalization + surface-role rendition + measured anchor，以及 request-shaped card、missing per-output dimensions、edited-output ratio drift 和 passive-measurement ambiguity 反例 | `ADAPT_TO_LIBTV` | 为 LibTV 分开 media/output/request/frame/measured/rendition/editor/export authority，保留 source-shaped landscape node，定义 fit transform、mixed-ratio switch 和 fresh anchor | `OC-081..090`、`OC-TR-019`；`DEC-039`；`LIBTV-PAR-001/002/008/009/011/014/015/016`；`LIBTV-FIX-LOCAL-MEDIA-RENDITION-01`；`LIBTV-VR-023` | static/design complete；runtime fragmented/source ratio-diverse parity gated；不移植 fixed card/request aspect/visual/schema，不把 measured/thumbnail 当 resize/intrinsic，也不实现 source-unconfirmed generic resize |
...
| 把 Open Canvas fixed card/request aspect 或 cover/contain 样式直接当 LibTV node frame policy | 上游 frame 跟 request aspect，当前 LibTV landscape source 更接近 media-shaped；surface fit 也不等于 intrinsic geometry | 分开 output/request/frame/measured/rendition；按 LibTV source 和具名 surface profile 决定 |
| 因 node width/height 可序列化或 measured 可读取就增加 resize/history | passive measurement 只证明当前 render rect；fixed upstream 也没有已证 generic resize intent | measurement 只刷新 anchor；semantic resize 继续 source/product-gated并需具名 transaction |
| 在 cover-cropped visible rect 上记录 marks 后宣称 full-media edit | 没有 content-box/crop transform 时，normalized visible coordinate 与 intrinsic coordinate 不相同 | 先声明 `FULL_INTRINSIC` 或 `VISIBLE_RENDER`，记录 transform baseline 并由 fixture round-trip 验证 |
...
Open Canvas 对本项目最有价值的是七类工程方法：坐标域/空间 owner 显式化、稳定身份与请求投影分离、状态生命周期分层、结构化 graph transaction、current owner 收敛、media bytes/asset/reference/resource lease 分权，以及 media/output/frame/rendition/measurement 分权。它最不适合作为 LibTV 视觉、产品文案、provider 能力、上传规格、节点 frame policy 和保存语义的替代来源。
