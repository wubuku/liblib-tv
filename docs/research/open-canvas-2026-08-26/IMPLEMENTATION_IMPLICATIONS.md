...
多 zoom 复测进一步拆开两种垂直合同：底部面板 gap 在 28%/50% 都是 `16 * zoom`；顶部工具条尺寸与中心稳定，但到节点顶部的 gap 从约 `16.794px` 变为 `22px`，不能继续把 clone `NodeToolbar offset=16` 当作源站完整实现。100% 离屏节点还出现 virtualization 卸载边界。详见 [`LIBTV_OVERLAY_MULTIZOOM_MATRIX.md`](LIBTV_OVERLAY_MULTIZOOM_MATRIX.md)。
...
当前最合理的下一步不是立即编码，而是继续补齐顶部 toolbar gap 的源码解释、元素编辑/旋转的无提交状态，以及专用空白项目中的 AutoLink ghost 视觉，再由用户授权 Batch A/A2。28%/50% 双浮层、100% virtualization boundary、preview、空 annotate、六动作 bundle 状态和 AutoLink 静态状态链已经完成；图层分离 live 取证必须等待任务/积分授权。具体视觉克隆仍应以 LibTV 源站证据为最高优先级。
