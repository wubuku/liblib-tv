...
2. 只现场打开静态处理路径能够证明不会提交任务的 `预览` 和空绘制 `标注`，读取 DOM 后立即关闭。
...
未点击 `元素编辑 / 图层分离 / 旋转 / 下载`，未绘制标注，未点击保存，未创建节点，未上传、生成或改写画布。bundle 中的 minified 标识符只用于还原状态链，本文不保存大段上游代码。
...
|---|---|---|---|---|---|
| 元素编辑 | 上报 `图片编辑`，设置 `ElementTool.EditElements` | `InteractiveImageEditMode`，点选/框选/画笔工具 | 点击生成后创建或复用输出节点，并通过 task gateway 提交 | 未点击 | 中：进入本地草稿；提交为高 |
...
| 旋转 | 有图片时设置 `ElementTool.RotateOrMirror` | 本地旋转/镜像编辑；退出脏状态出现保存/丢弃 modal | 保存可能上传旋转结果，并更新当前节点或创建输出 | 未点击 | 中到高 |
...
| 优先级 | 项目 | 安全前提 |
|---|---|---|
| P0 | 元素编辑实际 active toolbar/panel 几何 | 只进入并立即关闭仍会产生 analytics；需明确允许 |
| P0 | 旋转无修改进入/退出与 dirty modal | 不保存、不上传 |
