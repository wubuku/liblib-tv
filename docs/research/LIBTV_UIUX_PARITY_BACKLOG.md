...
| - | `LIBTV-PAR-014` | 媒体接入、asset/reference 与 temporary resource lifecycle correctness | 5 | 4 | 2 | 5 | `DESIGN_FIRST`；Batch 450 Slice A（十项入口注册表 + §8.1 有序校验 + 稳定 reason 族）、Batch 451 Slice B（instance 租约台账：release 恰好一次/转移盖戳；fake materializer：ok/duplicate/fail/stale 确定性结局）已关闭；Slice C（Add Resource 多文件纵切）待实施，source parity partial |
