...
| - | `LIBTV-PAR-015` | foreground editor session、local history、commit/close 与 inert-control honesty | 5 | 5 | 2 | 5 | `DESIGN_FIRST`；Batch 445 关闭 Slice A（纯 profile/session/history 模型：十项注册表、状态机 reducer、归一/脏检查、历史预算/合并）；Slice B/C 待实施，source parity partial |
