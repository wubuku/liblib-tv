...
| - | `LIBTV-PAR-015` | foreground editor session、local history、commit/close 与 inert-control honesty | 5 | 5 | 2 | 5 | `DESIGN_FIRST`；Batch 445 Slice A（纯 profile/session/history 模型）、Batch 446 Slice B（equality-aware graph commit adapter）、Batch 447 Slice C 子集（RECORD_EDITOR 指纹幂等：subtitle/picture 相同重提交 no-op）已关闭；Slice D 命令诚实度与 REQUEST_DRAFT 描述符待实施，source parity partial |
