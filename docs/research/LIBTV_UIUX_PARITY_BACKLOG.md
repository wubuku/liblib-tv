...
| - | `LIBTV-PAR-015` | foreground editor session、local history、commit/close 与 inert-control honesty | 5 | 5 | 2 | 5 | `DESIGN_FIRST`；Batch 445 Slice A（纯 profile/session/history 模型）、Batch 446 Slice B（equality-aware graph commit adapter：generation 计数器 + 四段校验 + 命名结果、accepted 恰一条历史）已关闭；Slice C（迁移现有编辑器）待实施，source parity partial |
