...
| - | `LIBTV-PAR-014` | 媒体接入、asset/reference 与 temporary resource lifecycle correctness | 5 | 4 | 2 | 5 | `DESIGN_FIRST`；Batch 450 Slice A（十项入口注册表 + §8.1 有序校验 + 稳定 reason 族）、Batch 451 Slice B（租约台账 + fake materializer）、Batch 453 Slice C（Add Resource 多文件纵切）、Batch 454 Slice D（引用 attach）、Batch 455 Slice E（Shot 源生命周期）、Batch 456 Slice F（Director data/blob 收敛：字节预算 + DIRECTOR_WORKSPACE 租约 + delete/undo 组合）已关闭——clone 侧 runtime 切片 A–F 全部完成；Slice G 为授权后真实 materializer，source parity partial |
| - | `LIBTV-PAR-015` | foreground editor session、local history、commit/close 与 inert-control honesty | 5 | 5 | 2 | 5 | `DESIGN_FIRST`；Batch 445 Slice A（纯 profile/session/history 模型）、Batch 446 Slice B（equality-aware graph commit adapter）、Batch 447 Slice C 子集（RECORD_EDITOR 指纹幂等）、Batch 448 Slice D（命令诚实度 pass）、Batch 449 Slice E（fake operation acceptor：冻结描述符 + 完成时 owner 再校验 + 卸载取消，四 owner-bearing 定时器迁移）已关闭；Slice F/G/H 为授权后范围，source parity partial |
| - | `LIBTV-PAR-016` | media output、aspect、node frame、surface rendition、measurement 与 editor-coordinate correctness | 5 | 5 | 2 | 5 | `DESIGN_FIRST`；Batch 441 Slice A（权威分类 + 冲突诊断）、Batch 442 Slice B（aspect-aware 派生帧政策）、Batch 443 Slice C（编辑器变换 + 基线修订 + 彩色标记 fixture 验证）、Batch 444 Slice D（per-output 身份 + 选择重排，历史出圈）已关闭——clone 侧 runtime 切片 A–D 全部完成；Slice E source-gated，source ratio-diverse parity gated |
...
其中 React Flow framework delta 已完成独立细化并形成 focused runtime pass，权威入口是 [`LIBTV_REACT_FLOW_CHANGE_ROUTING_CONTRACT.md`](LIBTV_REACT_FLOW_CHANGE_ROUTING_CONTRACT.md)：两个项目共同的 12.11.1 variant/reducer 语义、Open Canvas current-state 正面模式与 generic acceptance 反例、clone render-closure/whole-array/mixed-batch 风险、T0 selection、T1 existing-node position/passive measurement、semantic reroute、runtime-field sanitation、`LIBTV-FIX-LOCAL-REACT-FLOW-CHANGES-01` 和 `LIBTV-VR-016` 已定义。Batch 61 已实现 whole-batch classifier、current-snapshot routing、edge session selection 和 semantic zero-partial reject，并通过真实 drag/selection/delete、synthetic corpus、相邻回归与项目门禁；node resize/reconnect 和 portable document 全面 sanitation 保持未完成。

Viewport/coordinate/gesture/placement 也已完成 fixed audit 与正式设计，权威入口是 [`LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md`](LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md)：actual React Flow host、six coordinate domains、live/stable/bootstrap/target phases、gesture/operation owner、entry-specific placement、resize reconciliation、`LIBTV-FIX-LOCAL-VIEWPORT-COORDINATE-01` 和 `LIBTV-VR-020` 已定义。Batch 63 已关闭 actual-host default add；Batch 64 已关闭 Asset drawer host-center anchor；Batch 65 已关闭 demo responsive preset 覆盖 stable viewport、A/B stored viewport restore 和 current/old canvas callback guard，并验证 desktop/mobile、projection echo、invalid zero mutation和相邻 Batch 6/7/16/18/19/61/62/63/64。三批横跨 `PAR-001/002` overlay、`PAR-007` navigation、`PAR-008` graph placement 和 `PAR-011` canvas/UI owner。Runtime 仍缺 full live/stable endpoint、generic host epoch/generation owner、browser resize anchor、derived/duplicate/organize/selected-overlay composition 与 full fixture，source exact add/fit/resize/drop 继续 gated。
...
foreground editor session design (PAR-015)
