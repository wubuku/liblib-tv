...
| - | `LIBTV-PAR-014` | 媒体接入、asset/reference 与 temporary resource lifecycle correctness | 5 | 4 | 2 | 5 | `DESIGN_FIRST`；Batch 450 Slice A（十项入口注册表 + §8.1 有序校验 + 稳定 reason 族）、Batch 451 Slice B（租约台账 + fake materializer）、Batch 453 Slice C（Add Resource 多文件纵切：真实选择器入口 + provisional cohort 单次图事务 + 稳定 reason 反馈）已关闭；Slice D/E/F 待实施，source parity partial |
