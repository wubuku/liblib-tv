...
| 64 | Asset drawer host-resize anchor preservation | [`liblib-canvas-batch64-2026-08-27/`](liblib-canvas-batch64-2026-08-27/) `PLAN_RECORDED`，计划统一 toolbar/X/Canvas-context drawer action，在 current owner 下把旧 host center flow point 保持到新 host center；source exact policy、通用 resize observer 与 live/stable split 保持后续 |
