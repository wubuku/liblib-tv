...
> 状态：计划已落档（2026-08-26）。本批把当前图片工具条的
> `标注` 入口推进为 source-backed 的空 authoring state，不实现绘制保存、
...
- [`../components/ImageAnnotateMode.spec.md`](../components/ImageAnnotateMode.spec.md)：
  组件、状态和几何合同；
- `IMPLEMENTATION.md`：实施、验证、截图和提交结果；
- `SCREENSHOT_ANALYSIS.md`：本批截图唯一识图记录。
