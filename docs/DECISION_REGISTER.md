...
**影响：** Batch 63 已按本决策实现 actual-host default add：page 测量当前 React Flow host、调用 `screenToFlowPosition()`，store 按 graph dimensions 提交 Add Node/Character 的 one-step transaction。Batch 64 又实现 Asset drawer open/close/X/Canvas-context 的 host-center anchor preservation，并以 operation、canvas、instance 和 captured viewport guard 防止局部 stale commit；viewport reconcile 不进入 graph history。窄桌面工具条碰撞只在 screen space clamp，不改 graph anchor。两批都不移植 Open Canvas Quick Add/drop/pending connection、菜单尺寸、缩放范围、overlay DOM 或 persistence。LibTV source 的 exact add/drawer anchor、animation、auto-pan、fit/zoom、browser resize、drag cancel、organize framing 和 mobile degradation继续留在 decision queue；live/stable viewport、generic generation/host epoch owner 与 full `LIBTV-VR-020` 仍需独立授权。FrameOS React Flow viewport 与 Director 3D viewport 继续隔离。

**依据：** [`LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md`](research/LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md`](research/LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md)、[`liblib-canvas-batch63-2026-08-27/`](research/liblib-canvas-batch63-2026-08-27/)、[`liblib-canvas-batch64-2026-08-27/`](research/liblib-canvas-batch64-2026-08-27/)、[`LibTVOverlayPositioning.contract.md`](research/components/LibTVOverlayPositioning.contract.md)、Open Canvas `OC-053..060` fixed chain。
