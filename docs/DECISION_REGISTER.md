...
**影响：** Batch 63 已按本决策实现 actual-host default add 的 focused runtime slice：page 测量当前 React Flow host、调用 `screenToFlowPosition()`，store 按 graph dimensions 提交 Add Node/Character 的 one-step transaction；Asset drawer 在该 transient workflow 中保持 layout owner。它不移植 Open Canvas Quick Add/drop/pending connection、菜单尺寸、缩放范围、overlay DOM 或 persistence。LibTV source 的 exact add anchor/auto-pan/drawer coexistence、fit/zoom、panel resize、drag cancel、organize framing 和 mobile degradation继续留在 decision queue；live/stable viewport、generation/host epoch owner 与 full `LIBTV-VR-020` 仍需独立授权。FrameOS React Flow viewport 与 Director 3D viewport 继续隔离。

**依据：** [`LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md`](research/LIBTV_VIEWPORT_COORDINATE_GESTURE_STATIC_AUDIT_2026-08-27.md)、[`LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md`](research/LIBTV_VIEWPORT_COORDINATE_PLACEMENT_CONTRACT.md)、[`liblib-canvas-batch63-2026-08-27/`](research/liblib-canvas-batch63-2026-08-27/)、[`LibTVOverlayPositioning.contract.md`](research/components/LibTVOverlayPositioning.contract.md)、Open Canvas `OC-053..060` fixed chain。
