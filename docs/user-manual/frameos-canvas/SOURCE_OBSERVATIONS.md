...
- **本地上传注入尝试 (2026-09-24, Batch 218 探索)**：通过 DataTransfer 合成事件向
  素材库隐藏 input 注入真实 PNG 字节并派发 change——应用接收事件并清空输入，
  但文件夹仍“暂无数据”（上传未持久化）。结论：真实上传需要受信任的文件选择器
  事件，合成注入不足以完成流程，“实际上传流程未验证”结论维持。
