...
- **仍不可采样**：⌘V 粘贴链路（剪贴板权限）、对齐辅助线吸附（自动化拖拽限制）、
  触控板手势。
- **内容图片节点富工具条 (2026-09-24 采样)**：有内容的图片节点选中时，节点上方
  显示富工具条：⛶(全屏) / 下载 / ⭐收藏 / 超清 / 720全景 / 打光 / 改图 / 裁剪 /
  标注 / 宫格切分 ∨（下拉）。空图片节点无此工具条（Batch 172/204 一致）。
  工具条为瞬态显示，鼠标移开即消失。克隆待实现 (Batch 218 候选)。
...
- **内容音频节点渲染 (2026-09-25 采样，Batch 222)**：向“本地上传”input 注入
  程序生成的 0.6s 正弦 wav（DataTransfer + change 事件，无真实文件、无付费动作），
  源站创建音频节点并上传至 media.frameos.cn 素材库。内容态结构：260x160 卡片，
  浮动标题 = 文件名去扩展名（ri-music-2-line 图标）；`.audio-wrap` = 上部居中
  音乐图标（.audio-visual aria-hidden）+ 底部 plyr 播放器行 [播放/暂停 |
  当前时间 00:00 | 进度 seek range | 总时长]（`<audio preload=metadata>`）；
  右上角 替换内容 按钮；**仅右侧 handle**（无左 target）。选中：工具条 =
  [下载, 收藏] 两个 icon 按钮（**无全屏查看**），**无提示词面板**（空音频节点
  保留 Batch 213 面板）。探针节点已删除（源站节点删除无确认弹层，Backspace
  直接删）。截图：frameos/frameos-audio-content-node-selected.png。
- **素材库 添加参考素材行为 (2026-09-25 采样，Batch 224)**：选中库内素材
  （探针 wav 仍在 上传素材，卡片显示时长徽章 0.6s + 创建者名）后点
  「添加参考素材」= 按所选素材在画布**创建带内容节点**（音频素材→音频节点
  带播放器，标题=素材名），**无连线**、不自动选中；随后**素材库关闭**、选中
  清零。0 选中时按钮禁用。素材库树顶级含 全部素材/我的收藏；筛选行含
  收藏/创建者/创建时间。
- **内容媒体节点统一语义 (2026-09-25 采样，Batch 225，mp4 探针上传)**：
  新版源站对**有内容的媒体节点**（图片有图/视频/音频有料）选中态统一为
  **富工具条 + 无面板 + 仅右侧 handle**；**空节点**为 生成面板 + 左右双
  handle + 无/弱工具条。逐项：内容视频 300x172 = OSS 快照封面
  (`?x-oss-process=video/snapshot…`) + 中心播放按钮 + **右下角**替换内容 +
  **左下角时长徽章** (00:04)；选中工具条九项 = 全屏查看/下载/收藏/剪辑/裁剪/
  音视频分离/超清/去字幕/片段重拍（前三 icon，后六带小图标文本），**覆盖
  Batch 171 的旧两项采样**（2026-09-23 旧版）。内容图片选中 = 十项工具条 +
  **无面板**（修正 Batch 172/196 时代的内容图片面板认知）。空图片选中 =
  面板（聚焦/故事版/参考 + 帧界 O2.5 + 2K·16:9 + 100）+ 无工具条。空/非媒体
  节点（文本/3D导演台/空图片）均左右双 handle。右键视频菜单含
  复制⌘C/创建副本⌘D/删除⌫（与 Batch 170 一致）+ 快捷动作项。探针节点已删。
  截图：frameos/frameos-content-image-selected-2026-09-25.png、
  frameos/frameos-content-video-selected-2026-09-25.png。
- **内容图片右键菜单 (2026-09-25 采样，Batch 226 已实现)**：新版源站
  节点右键菜单**中间行按内容态区分**（DOM 逐行）：
  - 内容图片：复制⌘C / 复制图片 / 创建副本⌘D / （线）/ **设置为资产图** /
    （线）/ 删除⌫（红色）
  - 空图片：复制⌘C / 复制图片(禁用) / 创建副本⌘D / （线）/ **重新生成(禁用)** /
    （线）/ 删除⌫
  - 3D导演台（非媒体）：复制⌘C / 创建副本⌘D / （线）/ 重新生成(禁用) / （线）/
    删除⌫
  与 Batch 170 的旧版 5 行采样相比新增「设置为资产图」；中间行两侧各一条
  分隔线。文本节点菜单按非媒体形态推断（未单独采样）。
- **内容视频右键菜单 (2026-09-25 采样，Batch 228 已实现)**：内容视频节点
  菜单为**极简四态**：复制⌘C / 创建副本⌘D / （线）/ 删除⌫ —— **无中间行**
  （无 设置为资产图/重新生成/复制图片）。「设置为资产图」为**图片专属**；
  音频菜单未单独采样，克隆按视频同规则处理（[复制,创建副本,删除]）。
  探针节点已删。
- **图片灯箱 (2026-09-25 采样，Batch 227 已实现)**：内容图片工具条 ⛶全屏查看
  打开全屏灯箱（`.lightbox-chrome`）：近黑背景（画布变暗衬底）+ 居中 contain
  图片 + **顶部中央控制条 [放大 缩小 重置 | 下载]**（divider 分隔）+
  **右上角圆形 × 关闭**；Esc 也可关闭。源站按钮无 aria-label。灯箱仅查看，
  不改动画布。截图：frameos/frameos-image-lightbox-2026-09-25.png。
- **多选框选 + 成组工具条 (2026-09-25 采样，Batch 229 已实现)**：空白处
  **左键拖拽 = 框选**（非平移；真实拖拽选中 4 节点），出现 `.group-toolbar`：
  **[成组 (ri-group-line) | 批量下载 (ri-download-2-line)]**，高 36，位于选中
  集合包围盒上方 15px、水平居中；多选时单节点工具条隐藏。**成组点击行为未
  采样到**（采样时工具条滚出视口，点击无效；克隆为 mock 提示）。框选后点击
  选区包围盒内部不清除选择（xyflow 选区 overlay 拦截），点框外空白清除。
  合成 pointer/shift/meta 事件无法触发框选，需真实拖拽。截图：
  frameos/frameos-multiselect-group-toolbar-2026-09-25.png。
- **宫格切分/成组为潜在付费动作 (2026-09-26 记录)**：宫格切分会把图片切成
  子图（产出=生成，涉及积分），成组行为也未采样成功（多次真实拖拽后工具条
  不在视口/输入路径超时）——两者克隆均为 mock 提示，**采样时永不可点击**，
  避免消耗用户积分。视频全屏查看灯箱亦未单独采样（Batch 230 按图片灯箱
  同族推断实现：真视频源渲染播放器，封面源渲染封面图）。
- **替换内容按钮缩放 (2026-09-26 采样，Batch 236 已实现)**：源站节点
  替换内容按钮尺寸随画布缩放线性变化——100% 时 30×30px，75% 时 ≈22.7px
  （30 × zoom；DOM CSS 变量 `--node-replace-btn-size: calc(18px *
  var(--canvas-zoom))` 为图标基准，实测渲染含内边距）。克隆图片/视频/音频
  替换按钮统一改为 `30 × zoom`（含 inset/icon 同步缩放）。
- **视频面板模式 tab (2026-09-26 采样，Batch 240)**：首尾帧/视频编辑 tab 点击
  仅切换 active 态，空节点下面板其余内容不变（无新增插槽）。克隆 Batch 239
  实现已一致。探针节点已删（画布恢复 6 节点基线）。
- **成组点击最终状态 (2026-09-26，Batch 242)**：多次真实拖拽重试后源站拖拽
  输入路径不稳定（框选无法复现），成组点击效果维持**未采样**；克隆保持 mock
  提示。同批为克隆新 UI 存档 5 张渲染记录截图
  （clone-content-image-toolbar / clone-image-lightbox / clone-group-toolbar /
  clone-empty-video-panel / clone-context-menu-content-image，2026-09-26），
  并修正克隆成组工具条的文字换行问题（内容自适应宽度 + nowrap）。
- **帮助面板复检 (2026-09-26，Batch 245)**：源站帮助面板（.shortcuts-panel）
  四节逐字与克隆一致（创作/缩放/移动画布/其他，含剪切⌘X、粘贴⌘V、原地复制
  ⌘D、拖拽复制⌥、保存⌘S、小地图M、帮助?、取消选中Esc），**无漂移**。注意：
  ①"移动画布：左键拖动（平移）"为源站自身滞后文本——实测左键拖动=框选
  （Batch 229）；②"双击节点聚焦填满视口"同为滞后声明（Batch 225 已记录）；
  ③克隆键盘处理器已实现面板声明的全部快捷键。远程关闭该面板失败（输入路径
  降级），面板为纯查看器不影响数据。截图：frameos-help-panel-2026-09-26.png。
