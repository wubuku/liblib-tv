...
- **内容图片右键菜单 (2026-09-25 采样，Batch 226 待实施)**：新版源站对
  **有内容的图片节点**右键菜单行为（`.context-menu`，DOM 逐行）：
  复制⌘C / 复制图片 / 创建副本⌘D / （分隔线）/ **设置为资产图** / （分隔线）/
  删除⌫ —— 与 Batch 170 的旧版 5 行菜单（复制/复制图片/创建副本/删除 +
  2 禁用行）相比新增「设置为资产图」行；禁用态逐行未复核。
  **Batch 226 剩余计划**：①补采 空图片/文本/内容视频 右键菜单逐行 + 禁用态；
  ②克隆 `FrameosContextMenu` 按节点类型/内容态出差异化菜单行（至少补
  「设置为资产图」，mock 行为）；③验证器 + 回归。注意： Batch 170/178 的
  菜单验证器基于旧版 5 行采样，改动时需同步复核。
