...
## 11. 2026-09-26 手册刷新（FrameOS 复刻循环 Batch 241）

源站 2026-09-24 版本更新引入"内容媒体统一语义"等变化，克隆侧 Batch 220-240
已完成对齐（见 `docs/research/frameos/BATCH_225_CHECKPOINT.md`），本手册以下
页面已同步刷新：

- `edit-selected-node.md`：选中态工具行按"有内容/空"区分（内容图片十项工具行、
  内容视频九项、内容音频两项；空节点才有提示词面板）；新增"视频节点的生成面板"
  （全能参考/首尾帧/视频编辑 + Wan 3.0 + 300 积分）与"图片/视频工具行与全屏灯箱"
  章节；清理聚焦模式段的重复段落。
- `duplicate-delete-history.md`：右键菜单按节点类型/内容态区分（设置为资产图、
  重新生成出现条件、视频/音频无中间行）。
- `navigate-canvas.md`：左键拖动空白 = 框选多选（成组/批量下载工具条）。
- `create-first-node.md`：空媒体节点创建后面板行为表述更新。
