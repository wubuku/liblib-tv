...
| Batch 223 | Full regression sweep (175 verifiers): 155 PASS, 20 pre-existing aged/historical FAIL (zero new regressions from Batches 172-222); npm run check baseline confirmed; stability consolidation pass |
...
| Batch 207 | script-v2 node type implemented (350×350 脚本生成器 card, minimal placeholder body — editor/panel unsampled) with createStoryScriptPair as an atomic one-history pair action (text pre-filled 剧本 + script-v2, no edge); story-script chip wired to it; the Agent-send sampling was shelved (page-hang + account-side session risk); verify-liblib-batch207.py (8 checks) |
| Batch 208 | script-v2 selected-state internals sampled — floating title bar above the card (doc icon + 脚本生成器 at top −28px, zoom-scaled in source), shell rounded-xl #171717 (Surface-Panel-background) with selected outline, left target handle with an 80px connect zone; selecting the node opens no video panel; ScriptV2Node rebuilt accordingly and batch207 gained a floating-header check (9 checks) |
| Batch 213 | yunjing menu rebuilt as a 23-card motion gallery per source sampling (189×208 cards, remote webp thumbnails tool/movement/N.webp, hover favorite, centered names) replacing the 12-item clone-shaped text menu; selection kept; batch146's 12-preset assertions migrated to the gallery; unused index var and img lint handled |
| Batch 214 | yunjing menu confirmed as a pure launcher — selected and unselected cards render identical classes/markers, pill stays 运镜 after selection; the clone's ✓ 已选 marker removed (menu shows no selection state); verify-liblib-batch214.py (6 checks) |
| Batch 215 | With an effect material connected the source's 特效 pill relabels to 替换 (replace semantics) — clone implements via new effectApplied/onEffectApplied props wired to the effect-card click; 参考/标记 stay inert with a material edge too; verify-liblib-batch215.py (4 checks) |
| Batch 216 | MAJOR CORRECTION + 参考 select mode: mouse clicks at y≈1106 were landing OUTSIDE the 826 viewport — the pills were never actually clicked (JS b.click() works); the 参考 pill opens a reference-select mode with a top-center banner 从画布或资产管理选择参考返回节点 (364×56, portaled) — clone implements via refSelectMode + data-reference-select-banner; verify-liblib-batch216.py (6 checks) |
...
| Batch 211 | Character library modal re-sampled in the rendering window — modal opens at 1280×710 @1920 (clone: 1304×731 from the 1440 sample); the size rule is not derivable from two viewports so the clone stays unchanged (open question); the consent gate and public tab behave per Batch 169 |
...
252 255 264 268 281 295 298 332 334 336 337 341 435 436 437 438 439 441 442 443 444 445 446 447 448 449 450 451 453 454 455 456
```

fixture/聚焦验收家族（Batch 510–527，确定性 corpus 与真实动作场景）：

```
510 513 514 515 516 517 518 521 522 527
```
